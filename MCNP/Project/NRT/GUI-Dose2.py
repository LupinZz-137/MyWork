...
class Plotter:
    ...
    def create_contour_plot(data_path, stl_lines, axis, title, xlim=None, ylim=None):
        try:
            ...
            # 绘制等高线
            levels = np.logspace(0, 9, num=10)
            contour = ax.contourf(X_grid, Y_grid, Z_grid, levels=levels, 
                    cmap="RdBu_r", norm=LogNorm(vmin=1e0, vmax=1e+9), extend='both') 

            # 绘制2.5 μSv/h等高线
            ax.contour(X_grid, Y_grid, Z_grid, levels=[2.5], colors=['red'], linewidths=2)
            ax.text(0.02, 1.0, '——2.5 uSv/h', color='red', fontsize=10, transform=ax.transAxes)
