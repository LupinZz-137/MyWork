...
class MCNPResultProcessor:
    def __init__(self, I_microA: float, irradiation_time_hours: float, cooling_time_hours: float = 0):
        ...
        self.cooling_time = cooling_time_hours * 3600
        self.results_cache = {}
    ...
    def calculate_cooled_atom_numbers(self, atom_numbers_at_EOL: dict) -> dict:
        if self.cooling_time <= 0:
            return atom_numbers_at_EOL.copy()
        
        try:
            inventory = rd.Inventory(atom_numbers_at_EOL)
            cooled_inventory = inventory.decay(self.cooling_time)
            return cooled_inventory.activities()
        except Exception as e:
            print(f"使用radioactivedecay库计算冷却衰变时出错: {e}")
            return self._simple_decay_calculation(atom_numbers_at_EOL)
    
    def _simple_decay_calculation(self, atom_numbers: dict) -> dict:
        cooled_atom_numbers = {}
        
        for nuclide, N0 in atom_numbers.items():
            lambda_val = self.get_decay_constant(nuclide)
            if lambda_val > 0:
                cooled_atom_numbers[nuclide] = N0 * np.exp(-lambda_val * self.cooling_time)
            else:
    ...
    def process_mcnp_file(self, filename: str, output_file: str = None) -> dict:
            ...
            try:
                ...
                for nuclide, atoms in atom_numbers.items():
                    ...
                    detailed_results.append({
                        ...
                        'atom_number_EOL': atoms,
                        'activity_EOL_Bq': atoms * lambda_val if not is_stable else 0.0,
                        'is_stable': is_stable,
                        'decay_constant': lambda_val,
        ...
        all_atom_numbers_cooled = self.calculate_cooled_atom_numbers(dict(all_atom_numbers_EOL))
        ...
        for nuclide, total_atoms_EOL in all_atom_numbers_EOL.items():
            ...
            if original_nuclide:
                ...
                is_stable = (lambda_val == 0)
                activity_EOL = total_atoms_EOL * lambda_val if not is_stable else 0.0
                total_atoms_cooled = all_atom_numbers_cooled.get(nuclide, 0)
                activity_cooled = total_atoms_cooled * lambda_val if not is_stable else 0.0
                ...
                summary_data.append({
                    ...
                    'total_atom_number_EOL': total_atoms_EOL,
                    'total_atom_number_cooled': total_atoms_cooled,
                    'total_activity_EOL_Bq': activity_EOL,
                    'total_activity_EOL_GBq': activity_EOL / 1e9,
                    'total_activity_cooled_Bq': activity_cooled,
                    'total_activity_cooled_GBq': activity_cooled / 1e9,
        ...
        summary_df = summary_df.sort_values('total_atom_number_EOL', ascending=False)
        ...
        print(f"\n原子数最多的前20个核素 (辐照结束时):")
        print("=" * 120)
        print(f"{'核素':<12} {'EOL原子数':<20} {'EOL活度(GBq)':<15} {'冷却后原子数':<20} {'冷却后活度(GBq)':<15} {'稳定性':<10} {'半衰期':<25}")
        print("-" * 120)
        
        for idx, row in summary_df.head(20).iterrows():
            print(f"{row['nuclide']:<12} {row['total_atom_number_EOL']:<20.2e} {row['total_activity_EOL_GBq']:<15.2e} "
                  f"{row['total_atom_number_cooled']:<20.2e} {row['total_activity_cooled_GBq']:<15.2e} "
        ...
        return {
            ...
            'all_atom_numbers_cooled': all_atom_numbers_cooled
        }

def main():
    print("MCNP结果处理器 - 包含冷却时间计算")
    print("=" * 60)
